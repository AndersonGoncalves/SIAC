...
namespace SiacWeb.Services
{
    public class AutonomoService
    {
        ...
        {
            return await _context.Autonomo.FirstOrDefaultAsync(obj => obj.Id == id);
        }

        public async Task<IPagedList<Autonomo>> FindAllAsync(int pagina)
        {
            return await _context.Autonomo.OrderBy(obj => obj.Id).ToPagedListAsync(pagina, Constantes.QuantidadeRegistrosPorPagina);
        }

        public async Task<IPagedList<Autonomo>> FindAsync(int pagina, string consulta)
        {
